...
- **N+1 queries on game data.** `Games.get_quest!/1`, `get_play!/1`, and the play LiveView load `:locations`, `:actions`, `:quest`, `:theme` via `Repo.preload`. New code paths that touch locations/actions/plays should preload rather than lazily fetch associations inside loops.
